...
def main() -> None:
    ...
    parser.add_argument(
        "--model-out", type=Path, default=Path("backend/artifacts/acv_pipeline.joblib")
    )
    parser.add_argument(
        "--features-output", type=Path, default=Path("outputs/acv/train_features.csv")
    )
    parser.add_argument(
        "--cv-results", type=Path, default=Path("outputs/acv/cv_results.json")
    )
